...
AddEventHandler('esx:spawnVehicle' , function(vehicleName)
	...
	if IsModelInCdimage(model) then
		...
		TriggerEvent('chat:addMessage' , { args = { '^1[SapoBase]', '¡Modelo de vehículo invalido!' } })
	end
end)
			...
			MySQL.Async.fetchAll('SELECT * FROM job_grades', {}, function(jobGrades)
				for k,v in ipairs(jobGrades) do
					if ESX.Jobs[v.job_name] then
						...
						print(('^5[Your-Life] [^3WARNING^7] Ignoring job grades for "%s" due to missing job^7'):format(v.job_name))
					end
				end
				for k2,v2 in pairs(ESX.Jobs) do
					if ESX.Table.SizeOf(v2.grades) == 0 then
						ESX.Jobs[v2.name] = nil
						print(('^5[Your-Life] [^3WARNING^7] Ignoring job "%s" due to no job grades found^7'):format(v2.name))
					end
				end
			end)
		end)
