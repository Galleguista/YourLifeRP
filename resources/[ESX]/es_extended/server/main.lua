...
function onPlayerJoined(playerId)
	...
	if steamhex then
		if ESX.GetPlayerFromIdentifier(identifier) then
			DropPlayer(playerId , ('[SapoBase]: La cuenta de steam ya se encuentra conectada en nuestro servidor, si esto es un error contacta con un staff lo antes posible.\nhttps://SapoBase \n\nSteamID: %s'):format(identifier))
		else
		...
		DropPlayer(playerId , '[SapoBase]: No se encontro Steam abierto.')
	end
end
...
function loadESXPlayer(identifier , playerId)
	...
	table.insert(tasks , function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier' , {
			...
			if ESX.DoesJobExist(job , grade) then
				...
				print(('[Your-Life] [^3WARNING^7] Ignoring invalid job for %s [job: %s, grade: %s]'):format(identifier , job , grade))
			...
			if result[1].inventory and result[1].inventory ~= '' then
				...
				for name , count in pairs(inventory) do
					...
					if item then
						...
						print(('[Your-Life] [^3WARNING^7] Ignoring invalid item "%s" for "%s"'):format(name , identifier))
					end
				end
			end
			...
			if result[1].position and result[1].position ~= '' then
				...
				print('^3[SapoBase]^7 ¡Un usuario nuevo ingresó al servidor y spawneo con éxito!')
	...
	Async.parallel(tasks , function(results)
		...
		print(('[Your-Life] [^2INFO^7] A player with name "%s^7" has connected to the server with assigned player id %s'):format(xPlayer.getName() , playerId))
...
AddEventHandler('chatMessage' , function(playerId , author , message)
	if message:sub(1 , 1) == '/' and playerId > 0 then
		...
		TriggerClientEvent('chat:addMessage' , playerId , { args = { '^1[SapoBase]', _U('commanderror_invalidcommand' , commandName) } })
	end
end)
